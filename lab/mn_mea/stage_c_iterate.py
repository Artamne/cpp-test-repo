...
#: Число не выдумано: §5.7 документа говорит, что фаза определена с точностью
#: до 2 pi и разворачивать её не требуется, поэтому шаг больше pi в одном бине
#: неотличим от своего дополнения до 2 pi и новых сведений не несёт.
STEP_MAX_RAD = math.pi
...
def power_floor(backend: Backend, h, floor_relative: float = POWER_FLOOR_RELATIVE) -> float:
    """Решение реализации №1: порог для |g|^2 под логарифмом.

    Принимает бэкенд и данные блока h(k,n); возвращает порог мощности
    (одно вещественное число), ниже которого P не опускается.

        порог = floor_relative * S_g / (M N)

    Полная энергия S_g (5-4) от phi не зависит (§5.2 документа: ПФ и умножение
    на экспоненту единичного модуля энергию сохраняют), поэтому берётся по
    равенству Парсеваля прямо из h, без лишнего БПФ:

        S_g = alpha^2 * M * sum_{k,n} |h(k,n)|^2

    Порог, взятый так, постоянен на все итерации блока и не вносит собственной
    зависимости от phi.
    """
    M = h.shape[0]
    N = h.shape[1]
    sum_h2 = float(backend.sum_real(backend.xp.abs(h) ** 2))
    S_g = backend.alpha**2 * M * sum_h2
...
def newton_update(
    ...
    Здесь два названных ограничения, и оба попадают в отчёт:
      ...
      * |шаг| > step_max — ограничивается. Это ловит и случай E'' -> +0,
        где Ньютон выдал бы огромный шаг.
    """
    xp = backend.xp
    bad = E_2 <= 0.0
...
def iterate_block(
    ...
    S_g = backend.alpha**2 * M * float(backend.sum_real(h_abs2))
    floor = floor_relative * S_g / (M * N)

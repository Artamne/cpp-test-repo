...
class Geometry:
    ...
    #: Шаг изображения по азимуту, метров на отсчёт. Это НЕ то же самое, что
    #: r_a: r_a — РАЗРЕШЕНИЕ, то есть ширина отклика точечной цели, а шаг —
    #: расстояние между соседними отсчётами. У критично дискретизованной
    #: картинки они совпадают, и тогда поле можно не задавать. На реальной
    #: записи не совпадают: шаг равен v/PRF, а разрешение получается шире из-за
    #: весового окна и запаса по полосе, обычно в 1,2-1,5 раза.
    #:
    #: Разделены они потому, что входят в РАЗНЫЕ формулы. Критерий (5-20)
    #: |Delta R| <= r_a/2 — про разрешение: остаточный ход дальности не должен
    #: выходить за половину отклика. Перевод (5-22) m_p = 2 x_p / шаг — про
    #: шаг: сколько отсчётов укладывается в блок. Одно число на обе роли
    #: ошибается ровно во столько раз, во сколько разрешение шире шага.
    r_a_step: float | None = None

    @property
    def azimuth_step(self) -> float:
        """Шаг по азимуту, метров на отсчёт. Если r_a_step не задан — берётся
        r_a: у критично дискретизованной картинки шаг и разрешение равны."""
        return self.r_a if self.r_a_step is None else self.r_a_step


#: Решение реализации №6: перехлёст окна блока, в долях его размера m_b.
#:
#: ВЗЯТ НОЛЬ — перехлёста нет. История этого числа длинная и поучительная.
#:
#: Замысел был такой: расфокусированная цель размазана шире своей плитки, и
#: без перехлёста её хвосты теряются при вырезании. Сначала 0,5 подобрали на
#: стенде, где вносимая ошибка ОДНА на всю сцену; такой стенд цену перехлёста
#: показать не мог в принципе. На изменчивом стенде 0,5 оказался вреден, и
#: причину назвал владелец: энтропия считалась по ВСЕМУ окну, то есть поправка
#: подгонялась и под соседскую землю, где по (5-27) поправка ДРУГАЯ. Это
#: исправлено (stage_c_iterate.core_mask): суммы по m идут только по
#: сердцевине, перехлёст даёт данные и не даёт голоса.
#:
#: После исправления вред исчез — но и пользы не осталось. Замер на ПЯТИ
#: изменчивых сценах:
#:
#:   перехлёст/нули   L    остаток            контраст
#:      0,00 / 0,00   43   0,7938 ±0,0905     32,88 ±1,39
#:      0,00 / 0,25   64   0,6762 ±0,0895     32,27 ±2,60
#:      0,50 / 0,50  129   0,7145 ±0,1107     32,77 ±1,35
#:
#: По контрасту все три НЕРАЗЛИЧИМЫ: разброс между сценами больше разницы
#: между настройками. По остатку фазы лучший — 0,00 / 0,25, и он же втрое
#: короче по длине преобразования, чем 0,50 / 0,50. Перехлёст не окупается.
#:
#: ПЕРЕСМОТРЕНО НА НАСТОЯЩЕЙ ЗАПИСИ. Стенд стыков блоков не видит: остаток
#: фазы меряется по бинам, контраст тонет в разбросе сцен. На срезе
#: владельца (22 блока по 273 строки, поправка 10…25 рад размаха) нарезка
#: без перехлёста оставляет на каждом стыке тёмную полосу: поправка — это
#: свёртка по азимуту на +-20 отсчётов, и крайним строкам сердцевины
#: не хватает соседей, которых в окне нет. Замер, средняя |g| по строкам
#: стыков к средней по остальным (1,00 — стыка не видно), и линейка по
#: восьми целям:
#:
#:   перехлёст   стык/фон   -3 дБ   -20 дБ   ISLR
#:   до счёта      0,997
#:      0,00       0,724     1,30    4,11    -4,10   полосы видны глазом
#:      0,25       0,993     1,10    4,53    -5,19   полос нет
#:
#: Взято 0,25: полос нет, главный лепесток уже, ISLR лучше; -20 дБ шире на
#: десятую элемента. Голоса перехлёст теперь имеет (маска сердцевины из
#: критерия убрана, см. stage_c_iterate.core_mask), и это допустимо:
#: начальная фаза у соседей общая (этап B без ИНС), а блок 15 м — соседняя
#: земля ошибку движения не меняет. Цена: L = 2 m_b вместо 1,5 m_b.
BLOCK_OVERLAP_FRACTION = 0.25

#: Решение реализации №6, часть вторая: нули с каждой стороны, в долях m_b.
#: Свёртка в (5-3) круговая; без запаса содержимое, уехавшее за верхний край,
#: выходит снизу и накладывается само на себя.
#:
#: 0,25 выбрано по остатку фазы: он лучше, чем у нарезки встык, в 4 сценах из
#: 5 (посценно +0,193, -0,098, +0,130, +0,169, +0,194), в среднем на 15 %.
#: Контраст этой разницы не видит — она тонет в разбросе между сценами.
#: Больше не берём: при 0,5 и выше остаток снова растёт.
#:
#: Цена: длина (5-3) выросла с m_b до 1,5 m_b, оба БПФ итерации во столько
#: же раз длиннее. Число БПФ на итерацию прежнее — два.
...
def block_sample_sizes(x_p: float, y_p: float, geom: Geometry) -> tuple[float, float]:
    ...
    """
    m_p = 2.0 * x_p / geom.azimuth_step  # (5-22) считает ОТСЧЁТЫ, значит шаг
        ...
        for m_k in range(1, M_k + 1):
            ...
            blocks.append(
                Block(
                    ...
                    n_start=n0,
                    n_stop=n1,
                    x_centre=((k0 + k1) / 2.0 - M / 2.0) * geom.azimuth_step,

...
def interpolated_cut(cut: np.ndarray, factor: int = AZIMUTH_OVERSAMPLE) -> np.ndarray:
    ...
    то есть ровно та же сумма, но сетка изображения гуще в factor раз. Ни
    предположений о полосе, ни сдвига полусетки здесь не возникает — в
    отличие от дополнения нулями в СПЕКТРЕ, где выбор места для бина Найквиста
    двусмыслен и уводит отклик с места. Это грабли, на которые я наступил.

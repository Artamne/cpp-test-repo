...
#:
#: ВНИМАНИЕ, история этого числа важнее самого числа, и в ней две ошибки.
#:
#: Сначала 0,5 было подобрано на стенде, где вносимая ошибка ОДНА на всю
#: сцену. Такой стенд цену перехлёста показать не мог в принципе.
#:
#: Потом, на изменчивом стенде, 0,5 дал контраст 31,27 против 34,32 у
#: книжной нарезки встык, и я снизил долю до 0,25. Это лечило симптом.
#: Настоящая причина была в другом, и на неё указал владелец: перехлёст
#: обязан давать ДАННЫЕ и не давать голоса в целевой функции. Энтропия же
#: считалась по всему окну, то есть поправка подгонялась и под соседскую
#: землю, где по (5-27) поправка ДРУГАЯ. Исправлено в stage_c_iterate:
#: суммы по m в (5-5), (5-14) и (5-19) идут только по сердцевине,
#: см. core_mask.
#:
#: После этого вред исчез. На трёх изменчивых сценах 0,5 и 0,0 по контрасту
#: равны в пределах 0,3 % (33,63 против 33,74), а остаток фазы у 0,5 лучше
#: на 8 % (0,6766 против 0,7333). Взято 0,5 по остатку; выигрыш скромный и
#: за большее выдавать его нельзя.
...
#: Доля та же, 0,5, и по той же причине, что перехлёст: после запрета
#: перехлёсту голосовать в целевой функции вред пропал. Дальше 0,5 не
#: берём: при 1,0 контраст падает (32,41 против 33,63) — лишние нули
#: удлиняют доплеровскую ось и дают вектору фазы свободу, которая уходит
#: на подгонку под спекл. Как объяснение это НЕ проверено, проверен факт.

...
Порядок функций — порядок выполнения:
    ...
    block_data                                    плитка блока обратно в h
...
def block_data(backend, g_scene, block: Block):
    ...
    Принимает бэкенд, полное изображение сцены и Block; возвращает массив
    (m_b, n_b) РАЗМЕРОМ В БЛОК: m_b = m_stop - m_start бинов доплера,
    n_b = n_stop - n_start стробов дальности.

    Блок — участок сцены (§3.3), поэтому из изображения вырезается его
    плитка, и она обратным ПФ по азимуту возвращается в область
    дальность-доплер. Преобразование берётся ПО БЛОКУ, длиной m_b: §5
    открывается словами «далее M, N — размеры блока, h(k,n) — его данные»,
    а (5-3) суммирует по k = 1…M с ядром e^{-j 2 pi k m / M}. Значит длина
    преобразования — размер блока, и энергия за границу блока выйти не
    может: свёртка круговая, всё остаётся внутри плитки.

    Полоса доплера при этом не сужается — сетка из m_b бинов накрывает ту же
    полосу PRF реже, с шагом PRF / m_b. Поэтому ось частот для (5-27) берётся
    у stage_b_initial.azimuth_frequency_axis(m_b, T_a, M), где M — апертура.

    Чего это стоит против преобразования по всей сцене — не мнение, а замер:

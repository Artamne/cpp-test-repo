...
@dataclass(frozen=True)
class Block:
    """Один блок: сквозной номер q_k по (5-26), его место в массиве h(k,n)
    и координаты его центра относительно центра сцены, в метрах."""
    ...
    def shape(self) -> tuple[int, int]:
        return (self.k_stop - self.k_start, self.n_stop - self.n_start)
...
def block_grid(M: int, N: int, M_k: int, N_k: int, geom: Geometry) -> list[Block]:
    """Сетка блоков целиком: нарезка массива h(k,n) на M_k x N_k плиток и
    нумерация каждой по (5-26).

    Принимает размеры массива, число блоков по осям и Geometry;

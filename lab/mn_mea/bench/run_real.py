...
#: На сколько замеренное разрешение может расходиться с паспортным, прежде
#: чем об этом говорить. 20 % — запас на то, что медианное правило решения
#: №7 прихватывает скаты полосы, а не только её полку.
BAND_RESOLUTION_TOLERANCE = 0.2
...
def band_occupancy(backend, h, geom: Geometry) -> list[str]:
    ...
    # Занятость и разрешение связаны тождеством занятость = шаг / r_a, значит
    # из замеренной занятости разрешение ВЫВОДИТСЯ. Это единственный способ
    # проверить паспортное r_a по самим данным, а от него зависит размер
    # блока (5-20): x_p ~ r_a^2.
    if 0.0 < measured < 1.0:
        implied = geom.azimuth_step / measured
        out.append(
            f"  отсюда разрешение по азимуту {implied:.3f} м против "
            f"паспортных {geom.r_a:.3f} м")
        if abs(implied / geom.r_a - 1.0) > BAND_RESOLUTION_TOLERANCE:
            out.append(
                f"  ВНИМАНИЕ: расходятся в {geom.r_a / implied:.2f} раза. "
                f"Размер блока (5-20) идёт как r_a^2, значит блоков должно "
                f"быть примерно в {(geom.r_a / implied) ** 2:.1f} раза "
                f"{'больше' if implied < geom.r_a else 'меньше'}. "
                "Паспортное r_a стоит уточнить или задать сетку рукой")
    return out

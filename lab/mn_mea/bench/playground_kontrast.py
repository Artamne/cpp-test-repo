...
Варианты (все — над одним и тем же изображением, окно показа по процентилям):
    ...
    а  как сейчас: дБ, P5…P97
    б  дБ, P1…P99,5, гамма 0,8 — вытянуть тёмное
    в  амплитуда (не дБ), P2…P99 — «фотографический» вид
    г  многовзгляд по азимуту x7 (один элемент разрешения), дБ P5…P97 —
       спекл в 2,6 раза меньше, разрешение то же
    д  г + выравнивание по дальности (деление на сглаженный профиль
       яркости по стробам — диаграмма антенны)
    е  д + локальная нормировка в дБ (пиксель минус среднее по окну
       128x32 плюс общее среднее) — адаптивный контраст
    ж  д + фильтр Ли 5x5 по интенсивности — спекл ещё ниже, кромки целы

...
def main(out_dir: pathlib.Path) -> int:
    ...
    # --------------------------------------------------- 1а: разброс по сценам
    say("### 1а. Это число — ОДНА сцена. Вот что бывает на других")
    say()
    spread = V.experiment_seed_spread()
    say(f"Тот же опыт на {spread['seeds']} сценах, отличается только зерно "
        f"генератора:")
    say()
    say(f"* остаток: медиана **{spread['residual_median']:.3f} рад**, "
        f"от {spread['residual_min']:.3f} до {spread['residual_max']:.3f} — "
        f"разброс **{spread['residual_spread']:.0f} раз**")
    say(f"* сцена из пункта 1 — одна из удачных; типичная хуже неё примерно "
        f"в {spread['residual_median'] / max(main_case['residual_rms_rad'], 1e-12):.0f} раз")
    say()
    say("**Куда алгоритм приходит, если не в истину.** Энтропия (5-6) "
        "посчитана в трёх точках: при нуле, при ИСТИННОЙ фазе и при найденной.")
    say()
    say("| зерно | остаток, рад | S(0) | S(истина) | S(найдено) | контраст: доля идеала |")
    say("|---|---:|---:|---:|---:|---:|")
    for row in spread["rows"]:
        say(f"| {row['seed']} | {row['residual_rms_rad']:.3f} | "
            f"{row['S_zero']:.4f} | {row['S_true']:.4f} | {row['S_found']:.4f} | "
            f"{100 * row['share_of_ideal']:.0f}% |")
    say()
    say(f"**S(найдено) < S(истина) в {spread['deeper_than_truth']} случаях из "
        f"{spread['seeds']}**, медианный зазор {spread['gap_median']:.4f}. "
        "Итерации отрабатывают верно — они и должны идти в минимум (5-6). "
        "Минимум просто НЕ ЛЕЖИТ в истинной точке: у фазы M свободных "
        "параметров, и энтропию всегда можно опустить ниже истинной, стянув "
        "энергию в меньшее число пикселей. Контраст при этом выходит ВЫШЕ "
        f"идеального — медиана {100 * spread['share_of_ideal_median']:.0f} % "
        "от того, что даёт истина.")
    say()
    say("Практическое следствие, важное для реальной записи: **рост контраста "
        "сам по себе не доказывает, что фаза найдена верно.** Он доказывает "
        "только, что картинка стала концентрированнее — а это и есть то, что "
        "минимизируется. На стенде истина известна и расхождение видно; на "
        "записи её нет, и проверить нечем.")
    say()
    say("Что замерено и НЕ помогает: число стробов дальности (от 16 до 4096 "
        "остаток держится около 0,62 рад) и более мелкая нарезка по азимуту "
        "(от 1 блока к 32 остаток растёт с 0,54 до 0,88, а контраст уходит "
        "от идеала со 121 % до 159 %).")
    say()
    ...
    # ------------------------------------------------- сводка по способам
    say("## 5а. Что дал каждый способ — сводка")
    say()
    say("Один код, одни сцены. Каждая строка добавляет ОДНО изменение к "
        "предыдущей, поэтому видно вклад именно его. Верхняя строка — вообще "
        "без автофокуса, нижняя — идеальная сцена без внесённой ошибки; между "
        "ними всё и происходит. Колонки поиска фазы для этих двух пусты.")
    say()
    say("| способ | сошлось | остаток, рад | сдвиг, отсч. | S (5-6) | контраст | пик |")
    say("|---|---|---:|---:|---:|---:|---:|")
    for row in V.experiment_method_summary():
        mark = " ← сейчас" if row["is_current"] else ""
        conv = "—" if row["converged_blocks"] is None else f"{row['converged_blocks']:.1f}/12"
        res = "—" if row["mean_residual_rms_rad"] is None else f"{row['mean_residual_rms_rad']:.4f}"
        shift = "—" if row["mean_shift"] is None else f"{row['mean_shift']:.2f}"
        say(f"| {row['method']}{mark} | {conv} | {res} | {shift} | "
            f"{row['S']:.4f} | {row['contrast']:.2f} | {row['peak']:.1f} |")
    say()
    say("Снятие сдвига (№5) на S, контраст и пик не влияет ВОВСЕ — и не должно: "
        "все три меры не чувствительны к циклическому сдвигу, а он ровно то, "
        "что снимается. Его вклад виден только в колонке «сдвиг».")
    say()

    # --------------------------------- полный прогон на ИЗМЕНЧИВОЙ ошибке
    say("## 5в. Полный прогон на ПРОСТРАНСТВЕННО ИЗМЕНЧИВОЙ ошибке (§3)")
    say()
    say("Тот самый случай, ради которого книга и делит сцену на блоки. Ошибка "
        "вносится по модели самой книги (5-27): eps(x,y) = (D_x x + D_y y) f_a², "
        "своя для каждой точки местности, с теми же D_x, D_y, что считает этап B. "
        "Одним вектором фазы такую сцену уже не исправить.")
    say()
    split = V.experiment_space_variant_run()
    single = V.experiment_space_variant_run(blocks_override=1)
    say("| способ | сошлось | остаток, рад | контраст | пик |")
    say("|---|---|---:|---:|---:|")
    say(f"| разбиение по книге | {split['converged_blocks']}/{split['q']} | "
        f"{split['mean_residual_rms_rad']:.4f} | {split['contrast']:.2f} | {split['peak']:.1f} |")
    say(f"| один блок на всю сцену | {single['converged_blocks']}/1 | "
        f"{single['mean_residual_rms_rad']:.4f} | {single['contrast']:.2f} | {single['peak']:.1f} |")
    say(f"| идеальная сцена — потолок | — | — | {split['_ideal']['contrast']:.2f} | "
        f"{split['_ideal']['peak']:.1f} |")
    say()
    say(f"Разброс истины между блоками — до {split['pairwise_truth_spread_rad']:.3f} рад "
        f"СКО: вот это один блок исправить и не может. Здесь разбиение наконец "
        f"ВЫИГРЫВАЕТ, чего на неизменчивом стенде не было и быть не могло.")
    say()

    # ------------------------------------------- чего стенд НЕ проверяет
    say("## 5б. Чего этот стенд НЕ проверяет")
    say()
    say("Вносимая ошибка — ОДИН вектор на всю сцену: synthetic.phase_error "
        "домножает им все стробы дальности одинаково. Пространственной "
        "изменчивости, ради которой книга и делит сцену на блоки (§3), в "
        "данных нет вовсе. Видно это так: посчитать сцену ОДНИМ блоком во "
        "всю апертуру и сравнить с книжным разбиением.")
    say()
    inv = V.experiment_space_invariance()
    say("| способ | сошлось | остаток, рад | контраст | пик |")
    say("|---|---|---:|---:|---:|")
    one, split, ideal = inv["single_block"], inv["book_split"], inv["ideal"]
    say(f"| один блок на всю сцену (q = 1) | {'да' if one['converged'] else 'нет'} | "
        f"{one['residual_rms_rad']:.4f} | {one['contrast']:.2f} | {one['peak']:.1f} |")
    say(f"| разбиение по книге | {split['converged']} | "
        f"{split['residual_rms_rad']:.4f} | {split['contrast']:.2f} | {split['peak']:.1f} |")
    say(f"| идеальная сцена — потолок | — | — | {ideal['contrast']:.2f} | {ideal['peak']:.1f} |")
    say()
    say("Один блок упирается в потолок. Это НЕ довод против разбиения: это "
        "значит, что чинить разбиению на таком стенде нечего, и всякий вывод "
        "о качестве этапов A и B по этим числам будет о чём-то другом. "
        "См. README, «Чего в этой работе НЕТ», пункт 1.")
    say()
    ...
    # ------------------------------------------------------- решения §6
    say("## 8. Решения реализации (§6 задания — четыре; пятое, шестое и "
        "седьмое найдены в работе)")
    say()
    ...
    say("Доли даны в единицах m_b. Остаток фазы падает и дальше, но контраст "
        "собранной сцены при перехлёсте 1,0 чуть хуже, и выбрано 0,5 по этому "
        "числу. ПРИЧИНА НЕ УСТАНОВЛЕНА: напрашивающееся объяснение про "
        "пространственную изменчивость на этом стенде не проверяемо — "
        "вносимая ошибка одна на всю сцену. Догадка про подгонку под соседей "
        "тоже не подтвердилась: контраст сердцевины при 0,5 и 1,0 почти "
        "одинаков. Разница мелкая, около 3 %, на грани разброса.")
    say()
    ...
    say("### Решение №7: весовое окно по азимуту")
    say()
    say("У невзвешенной апертуры первый боковой лепесток равен -13,3 дБ — это "
        "её точное свойство, а не дефект. Рядом с яркой целью такие боковики "
        "закрывают слабую, и обычный размен в РСА — окно: боковики вниз ценой "
        "расширения главного лепестка. В книге взвешивания нет ни слова, "
        "поэтому по умолчанию окна НЕТ, (5-3) считается как написано.")
    say()
    say("Окно накладывается ТОЛЬКО в этапе D, на построение картинки: вывод "
        "(5-13) и (5-19) сделан для суммы без весов, и занеся вес внутрь цикла, "
        "пришлось бы выводить обе производные заново.")
    say()
    say("| окно | пик, отсч. | первый боковой | таблично | ширина по -3 дБ | потери по пику |")
    say("|---|---:|---:|---:|---:|---:|")
    книга = {"none": "-13,3", "hann": "-31,5", "hamming": "-42,7", "kaiser6": "-44,0"}
    for row in V.experiment_azimuth_window():
        mark = " ← по умолчанию" if row["is_default"] else ""
        say(f"| {row['window']}{mark} | {row['peak_position']:.3f} | "
            f"{row['sidelobe_db']:.2f} дБ | {книга.get(row['window'], '')} | "
            f"{row['width_samples']:.2f} | {row['peak_loss_db']:+.2f} дБ |")
    say()
    say(f"Цель стояла в {V.experiment_azimuth_window()[0]['target_position']:.3f} отсчёта. "
        "Пик НЕ ДВИГАЕТСЯ ни при каком окне — и не должен: окно расширяет "
        "главный лепесток, а положение цели не меняет. Это и есть проверка "
        "самого замера: пока восстановление среза шло по натуральному номеру "
        "бина вместо центрированного, пик уезжал, а уровни не сходились с "
        "табличными.")
    say()
    ...
    say("### Решение №7: пустые доплеровские бины")
    say()
    say("Книга молча считает доплеровскую ось занятой целиком. На записи "
        "занятая доля равна отношению ШАГА картинки к РАЗРЕШЕНИЮ: полоса "
        "доплера B_a = v/r_a, а частота повторения PRF = v/шаг. У владельца "
        "шаг 0,0464 м при разрешении 0,4 м — занято 11,6 % оси, то есть 39 "
        "бинов из 330 в блоке. Остальные несут только шум приёмника.")
    say()
    say("Что они делают без сторожа: E'' (5-19) в них мал, Ньютон (5-8) "
        "выдаёт огромный шаг, шаг упирается в предел STEP_MAX_RAD, и "
        "критерий (5-9) встаёт РОВНО на 2|sin(1/2)| = 0,9589 и держится там "
        "вечно. Это тот же предельный цикл, что был при пределе pi.")
    say()
    band_rows = V.experiment_signal_band()
    say(f"Замер на {band_rows[0]['seeds']} сценах, полоса задана долей; "
        "остаток считается только по занятым бинам:")
    say()
    say("| полоса | занято | нашло №7 | №7 | итераций | сошлось | (5-9) прибит | остаток, рад |")
    say("|---|---:|---:|---|---:|---:|---:|---:|")
    for row in band_rows:
        for label, tag in (("on", "вкл"), ("off", "выкл")):
            head = (f"| {row['fraction']:g} | {row['occupied_true']} | "
                    f"{row['live_on']} " if label == "on" else "|  |  |  ")
            say(head + f"| {tag} | {row[f'iterations_{label}']:.0f} | "
                f"{row[f'converged_{label}']}/{row['seeds']} | "
                f"{row[f'pinned_{label}']:.0f} | "
                f"{row[f'residual_{label}']:.3f} |")
    say()
    narrow = [r for r in band_rows if r["live_on"] < 330]
    if narrow:
        say(f"**Правило находит полосу точно**: "
            + ", ".join(f"{r['live_on']} из {r['occupied_true']}" for r in narrow)
            + ". Предельный цикл снимается: (5-9) прибит "
            f"{narrow[-1]['pinned_off']:.0f} итераций из "
            f"{narrow[-1]['iterations_off']:.0f} без правила и "
            f"{narrow[-1]['pinned_on']:.0f} с ним.")
        say()
        say("**Но точность правило НЕ улучшает** — остаток с ним и без него "
            "совпадает в пределах разброса между сценами. Оно чинит "
            "сходимость и стоимость, а не попадание в истину: в истину "
            "мешает попасть то, что описано в разделе 1а, и полоса тут ни "
            "при чём.")
        say()
    say("При доле 0,5 и выше правило само отключается: пустых бинов не "
        "большинство, медиана профиля попадает в сигнал, и разделять нечего. "
        "Поэтому на стенде, где ось занята целиком, оно не вмешивается вовсе "
        "— отчёт до его появления и после совпадает число в число.")
    say()

    say("### Решение №8: возвращается лучшая точка, а не последняя")
    say()
    say("Ньютон (5-8) с диагональным гессианом спуска НЕ гарантирует: "
        "направление `-E'/E''` построено без перекрёстных членов, и шаг "
        "может пройти мимо. Книга сторожа не ставит, но она и не говорит, "
        "какую точку считать ответом — сказано лишь, когда остановиться (5-9).")
    say()
    keep_rows = V.experiment_keep_best()
    say("| сцена | mu | итераций | из них вверх | вернули не последнюю | "
        "остаток: последняя | лучшая | лучше в |")
    say("|---|---:|---:|---:|---:|---:|---:|---:|")
    for row in keep_rows:
        say(f"| {row['scene']} | {row['mu']:g} | {row['iterations_total']} | "
            f"{row['entropy_rose']} | {row['kept_best']}/{row['seeds']} | "
            f"{row['residual_last']:.3f} | {row['residual_best']:.3f} | "
            f"{row['wins']}/{row['seeds']} |")
    say()
    rose = sum(r["entropy_rose"] for r in keep_rows)
    steps = sum(r["iterations_total"] for r in keep_rows)
    bad = sum(r["worse_than_start"] for r in keep_rows)
    say(f"Итого: энтропию увеличили **{rose} итераций из {steps}** "
        f"({100 * rose / max(steps, 1):.1f} %). Без решения №8 прогон "
        f"заканчивался ХУЖЕ, чем начинался, в {bad} случаях — именно это и "
        "наблюдалось на реальной записи, где в двух блоках из девяти "
        "энтропия за прогон выросла. С решением №8 «S после» не может "
        "превысить «S до» никогда.")
    say()
    say("Выигрыш по остатку скромный, 3-4 %, но он бесплатный: лишних БПФ "
        "нет, только копия массива фазы.")
    say()
